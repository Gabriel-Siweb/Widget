...
import com.example.widget_android.widget.ClockingWidget
...
object ClockingRepository {
    ...
    fun getState(context: Context): ClockingState {
        ...
        return ClockingState(
            mode = mode,
            currentStepIndex = prefs.getInt(KEY_STEP_INDEX, 0),
            isFinished = prefs.getBoolean(KEY_FINISHED, false),
    ...
    fun setMode(context: Context, mode: ClockingMode) {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ClockingWidget().updateAll(context)
        }
    }
